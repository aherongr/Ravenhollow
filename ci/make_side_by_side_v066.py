...
SAFE_ID = 'gr.ravenhollow.app.safe066'
...
g = re.sub(r'versionCode\s*=\s*\d+', 'versionCode = 6601', g)
g = re.sub(r'versionName\s*=\s*"[^"]+"', 'versionName = "0.6.6-safe6"', g)
...
m = re.sub(r'android:label="[^"]+"', 'android:label="Ravenhollow Safe 6"', m, count=1)
manifest.write_text(m, encoding='utf-8')

if BASE_AUTHORITY in manifest.read_text(encoding='utf-8'):
    raise SystemExit('Old FileProvider authority remains')
if SAFE_AUTHORITY not in manifest.read_text(encoding='utf-8'):
    raise SystemExit('Safe 6 authority missing')

print('Prepared Ravenhollow Safe 6')
